...
function huskiesLost(game) {
  if ((game["opponent"] == "UW") && (game["home_score"] > game["opponent_score"])) {
    return true;
  } else if ((game["home"] == "UW") && (game["home_score"] < game["opponent_score"])) {
    return true;
  }
  return false;
}
...
huskyGames2016.filter(huskiesLost).forEach(function(game) {
...
function mostYardsPassing(game1, game2) {
  if ((!game1["passing_yards"]) || (game1["passing_yards"] < game2["passing_yards"])) {
    return game2;
  }
  return game1;
}
