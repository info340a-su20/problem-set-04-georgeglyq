...
/*Passing in a string is different from passing in a number value.*/
...
*/
function compoundInterest(initialBalance, interestRate, numOfYears) {
  return initialBalance * Math.exp(interestRate * numOfYears);
}
...
for (let i = 0; i < suit.length; i++) {
  for (let j = 0; j < rank.length; j++) {
    let combined = {"suit":suit[i], "rank":rank[j]};
    deck.push(combined);
  }
}
console.log(deck);
...
function getHighCard(card) {
  let highest = card[0].rank;
  let result = card[0];
  for (let i in card) {
    if (card[i].rank > highest) {
      highest = card[i].rank;
      result = card[i];
    }
  }
